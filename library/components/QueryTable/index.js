...
const QueryTable = ({
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (error || !data || !metadata) {
        ...
        setAttributes([]);
        setSubtitle('');
        console.log('ERROR: unable to load data for query table'); // eslint-disable-line no-console
        return;
      }
      ...
      setSortingTypes(attribs.map(() => TableHeaderCell.SORTING_TYPE.NONE));

      if ('timeRange' in metadata)
        setSubtitle(formatSubtitle(metadata.timeRange));
    };
  ...
  const updateSortingType = (idx, nextSortingType) =>
    setSortingTypes(
      sortingTypes.map((_, i) =>
        i === idx ? nextSortingType : TableHeaderCell.SORTING_TYPE.NONE
      )
    );

  const formatSubtitle = ({ begin_time: begin, end_time: end }) =>
          ...
          <TableHeader>
            {attributes.map((attribute, i) => (
              <TableHeaderCell
                sortable
                sortingType={
                  sortingTypes[i] || TableHeaderCell.SORTING_TYPE.NONE
                }
                onClick={(_, { nextSortingType }) =>
                  updateSortingType(i, nextSortingType)
                }
                width="fit-content"
          ...
          {({ item }) => (
            <TableRow>
              {attributes.map((attribute, i) => (
                <TableRowCell key={i}>
                  {attribute === 'timestamp'
                    ? formatTimestamp(item[attribute])
                    : item[attribute]}
                </TableRowCell>
              ))}
            </TableRow>
